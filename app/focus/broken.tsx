...
export default function FocusBrokenScreen() {
  ...
  const penalty = parseInt(params.penalty ?? '0');

  const shakeAnim = useRef(new Animated.Value(0)).current;
  const fadeAnim = useRef(new Animated.Value(0)).current;
  const scaleAnim = useRef(new Animated.Value(1.2)).current;

  const message = SESSION_BREAK_MESSAGES[Math.floor(Math.random() * SESSION_BREAK_MESSAGES.length)];
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      <View style={styles.content}>
        ...
        <Animated.View style={[styles.messageSection, { opacity: fadeAnim }]}>
          <Text style={styles.brokenTitle}>Session Toot Gaya</Text>
          <Text style={styles.rudeMessage}>{message}</Text>
          ...
          <View style={styles.consequences}>
            <View style={styles.consequenceRow}>
              ...
              <Text style={styles.consequenceText}>
                {penalty > 0 ? `-${penalty} XP deducted` : 'XP penalized'}
              </Text>
            </View>
            <View style={styles.consequenceRow}>
              <MaterialIcons name="remove-circle" size={18} color={Colors.danger} />
        ...
        <Animated.View style={[styles.actions, { opacity: fadeAnim }]}>
          ...
          >
            ...
            <Text style={styles.tryAgainText}>Phir Try Karo</Text>
          </TouchableOpacity>
          <TouchableOpacity
