-- Defense in depth for the authenticated SECURITY DEFINER referral bonus RPC.
-- The app calls this only for the currently signed-in referee.
BEGIN;
...
BEGIN
  IF auth.uid() IS NULL OR auth.uid() <> p_referee_id THEN
    RAISE EXCEPTION 'Only the signed-in referee can process this referral bonus';
  END IF;
