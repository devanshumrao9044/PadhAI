-- Ensure every new Auth identity, including Google OAuth users, receives a
-- complete public profile without requiring a client-side profile insert.
-- Preserve referral validation and generated referral-code behavior.
...
BEGIN
  ...
  prefix := upper(substring(
    regexp_replace(profile_name, '[^a-zA-Z]', '', 'g'),
    1,
    4
  ));
